...
> **Depends on:** SPEC 02
> **Date:** 2026-08-13
> **Objective:** Agregar un modal en `/kids` para crear un niño con 3 campos obligatorios (nombre, fecha de nacimiento con máscara dd/mm/aaaa y sala) y 2 opcionales (alergias y notas médicas), que al guardar lo agrega en memoria al listado agrupado por sala (Sol, Tierra y Luna).

## Scope

**In:**

- Modal con los campos NOMBRE COMPLETO, FECHA DE NACIMIENTO, SALA, ALERGIAS (ETIQUETAS) y NOTAS MÉDICAS, replicando el estilo de `agregar-nino.dc.html`.
- Los 3 campos (nombre, fecha, sala) son obligatorios; errores inline bajo cada campo al pulsar Guardar.
- Alergias y notas médicas son opcionales: si están vacías, se guardan sin ellas.
...
- Se agrega `allergies?: string` (etiquetas, ej. "Maní, Lactosa") al tipo `Child`; las notas médicas se guardan en el campo `allergyNotes?: string` ya existente. Ambos opcionales.
...
3. **Modal en `components/kids/add-child-modal.tsx`.** Overlay con tarjeta al estilo de la referencia (fondo `#FBF4EC`, border `#ECE0D0`, radio 24px), header con Cancelar / "Agregar niño" / Guardar, y los 5 campos con sus labels (3 obligatorios + alergias y notas médicas opcionales). _Test:_ render aislado.
...
7. **Lógica de Guardar.** Validar, generar el `Child` (con alergias y notas médicas si se completaron), agregarlo al estado y cerrar el modal. _Test:_ el niño nuevo aparece en su sección con edad calculada.
...
- [ ] El modal muestra los 5 campos de la referencia (nombre, fecha, sala, alergias, notas médicas) y los botones Cancelar y Guardar.
- [ ] El dropdown de sala ofrece Sol (preseleccionada), Tierra y Luna.
- [ ] La fecha de nacimiento aplica máscara dd/mm/aaaa al escribir.
- [ ] Pulsar Guardar con campos obligatorios vacíos muestra errores inline bajo cada campo y no cierra el modal.
- [ ] Pulsar Guardar con los 3 obligatorios completos agrega el niño al listado y cierra el modal, aunque alergias y notas médicas estén vacías.
- [ ] Si se completan alergias y/o notas médicas, quedan guardadas en el `Child` (campos `allergies` / `allergyNotes`).
...
- **Sí:** Los 3 campos obligatorios son nombre, fecha y sala — los demás son opcionales.
- **Sí:** Alergias y notas médicas opcionales, fieles a la referencia — se guardan solo si se completan.
- **No:** Exigir alergias o notas médicas — solo los 3 obligatorios validan.
...
- Mostrar alergias como tags en la tarjeta del listado (se guardan en el dato, su visualización en tarjeta/perfil queda para otro spec).
